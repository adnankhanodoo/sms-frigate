...
# ============================================================
# SMS SENTINEL AI — CUSTOM CAMERA MANAGEMENT APIs
# ============================================================

RTSP_PATHS = {
    "dahua": {"main": "/cam/realmonitor?channel=1&subtype=0", "sub": "/cam/realmonitor?channel=1&subtype=1"},
    "hikvision": {"main": "/Streaming/Channels/{channel}01", "sub": "/Streaming/Channels/{channel}02"},
    "reolink": {"main": "/h264Preview_01_main", "sub": "/h264Preview_01_sub"},
    "tplink": {"main": "/stream1", "sub": "/stream2"},
    "uniview": {"main": "/media/video1", "sub": "/media/video2"},
    "axis": {"main": "/axis-media/media.amp", "sub": "/axis-media/media.amp?videocodec=h264"},
...
def _read_config():
    ...
    with open(config_file, "r") as f:
        return yaml.load(f), config_file, yaml


def _write_config(config_data, config_file, yaml):
    stream = StringIO()
...
@router.post("/cameras/add", dependencies=[Depends(require_role(["admin"]))])
async def add_camera_sms(request: Request):
    """Add camera. Brands: dahua, hikvision, reolink, tplink, uniview, axis, generic"""
    ...
    if not name or not ip:
        return JSONResponse(
            content={"success": False, "message": "name and ip are required"},
            status_code=400,
        )

    paths = RTSP_PATHS.get(brand, RTSP_PATHS["generic"])
    ep = urllib.parse.quote(password, safe="")
    eu = urllib.parse.quote(username, safe="")
    main_path = paths["main"].replace("{channel}", str(channel))
    sub_path = paths["sub"].replace("{channel}", str(channel))
    main_rtsp = f"rtsp://{eu}:{ep}@{ip}:{port}{main_path}"
    sub_rtsp = f"rtsp://{eu}:{ep}@{ip}:{port}{sub_path}"

    config_data, config_file, yaml = _read_config()

    if name in config_data.get("cameras", {}):
        return JSONResponse(
            content={"success": False, "message": f"Camera {name} already exists"},
            status_code=409,
        )

    if "go2rtc" not in config_data:
        config_data["go2rtc"] = {}
    if "streams" not in config_data["go2rtc"]:
        config_data["go2rtc"]["streams"] = {}

    config_data["go2rtc"]["streams"][name] = [main_rtsp]
    config_data["go2rtc"]["streams"][f"{name}_sub"] = [sub_rtsp]
    ...
    config_data["cameras"][name] = {
        ...
        "detect": {"enabled": True, "width": width, "height": height, "fps": fps},
        "record": {"enabled": True},
    ...
    return JSONResponse(
        content={
            "success": True,
            "message": f"Camera {name} added. Restart Frigate to apply.",
            "brand": brand,
            "go2rtc_main": f"rtsp://127.0.0.1:8554/{name}",
            "go2rtc_sub": f"rtsp://127.0.0.1:8554/{name}_sub",
        },
        status_code=201,
    )


@router.delete("/cameras/{camera_name}/remove", dependencies=[Depends(require_role(["admin"]))])
async def delete_camera_sms(camera_name: str):
    """Delete camera and its go2rtc streams."""
    config_data, config_file, yaml = _read_config()
    ...
    del config_data["cameras"][camera_name]
    streams = config_data.get("go2rtc", {}).get("streams", {})
    streams.pop(camera_name, None)
    streams.pop(f"{camera_name}_sub", None)
    _write_config(config_data, config_file, yaml)
    return JSONResponse(
        content={"success": True, "message": f"Camera {camera_name} deleted. Restart to apply."}
    )


@router.put("/cameras/{camera_name}/rename", dependencies=[Depends(require_role(["admin"]))])
async def rename_camera_sms(camera_name: str, request: Request):
    """Rename a camera."""
    data = await request.json()
    new_name = data.get("new_name")
    if not new_name:
        return JSONResponse(
            content={"success": False, "message": "new_name is required"},
            status_code=400,
        )
    config_data, config_file, yaml = _read_config()
    if camera_name not in config_data.get("cameras", {}):
    ...
    config_data["cameras"][new_name] = config_data["cameras"].pop(camera_name)
    streams = config_data.get("go2rtc", {}).get("streams", {})
    if camera_name in streams:
        streams[new_name] = streams.pop(camera_name)
    if f"{camera_name}_sub" in streams:
        streams[f"{new_name}_sub"] = streams.pop(f"{camera_name}_sub")
    _write_config(config_data, config_file, yaml)
    return JSONResponse(
        content={"success": True, "message": f"Camera renamed to {new_name}. Restart to apply."}
    )


@router.put("/cameras/{camera_name}/detection", dependencies=[Depends(require_role(["admin"]))])
async def toggle_detection_sms(camera_name: str, request: Request):
    """Enable/disable detection and set labels."""
    data = await request.json()
    ...
    config_data["cameras"][camera_name]["detect"]["enabled"] = enabled
    if labels:
        config_data["cameras"][camera_name]["objects"] = {"track": labels}
    _write_config(config_data, config_file, yaml)
    return JSONResponse(
        content={"success": True, "message": f"Detection {'enabled' if enabled else 'disabled'} for {camera_name}"}
    )


@router.put("/cameras/{camera_name}/audio", dependencies=[Depends(require_role(["admin"]))])
async def toggle_audio_sms(camera_name: str, request: Request):
    ...
    if "audio" not in config_data["cameras"][camera_name]:
        config_data["cameras"][camera_name]["audio"] = {}
    config_data["cameras"][camera_name]["audio"]["enabled"] = enabled
    _write_config(config_data, config_file, yaml)
    return JSONResponse(
        content={"success": True, "message": f"Audio {'enabled' if enabled else 'disabled'} for {camera_name}"}
    )


@router.get("/cameras/list", dependencies=[Depends(allow_any_authenticated())])
async def list_cameras_sms(request: Request):
    """Get all cameras with live status."""
    config_data, _, _ = _read_config()
    cameras = config_data.get("cameras", {})
    try:
        stats_resp = requests.get("http://127.0.0.1:5001/api/stats", timeout=3)
        stats = stats_resp.json().get("cameras", {})
    except Exception:
        stats = {}
    result = []
